...
entity Tc_Project_Addition_Info {
    ...
    key period_code         : String(30) not null @title : '기간코드';
        ...
        mtlmob_ref          : Association[0.. * ] to dp.Tc_Project
                                  on  mtlmob_ref.tenant_id        = tenant_id
                                  and mtlmob_ref.project_code     = project_code
                                  and mtlmob_ref.model_code       = model_code
                                  and mtlmob_ref.mcst_code        = mcst_code
                                  and mtlmob_ref.version_sequence = version_sequence
                                  and addition_type_code          = 'MTLLMOB'; //물동

        sales_price_ref     : Association[0.. * ] to dp.Tc_Project
                                  ...
                                  and sales_price_ref.mcst_code        = mcst_code
                                  and sales_price_ref.version_sequence = version_sequence
                                  and addition_type_code               = 'SALES_PRICE'; //판가

        prcs_cost_ref       : Association[0.. * ] to dp.Tc_Project
                                  on  prcs_cost_ref.tenant_id        = tenant_id
                                  and prcs_cost_ref.project_code     = project_code
                                  and prcs_cost_ref.model_code       = model_code
                                  and prcs_cost_ref.mcst_code        = mcst_code
                                  and prcs_cost_ref.version_sequence = version_sequence
                                  and addition_type_code             = 'PROCESSING_COST'; //가공비

        sgna_ref            : Association[0.. * ] to dp.Tc_Project
                                  on  sgna_ref.tenant_id        = tenant_id
                                  and sgna_ref.project_code     = project_code
                                  and sgna_ref.model_code       = model_code
                                  and sgna_ref.mcst_code        = mcst_code
                                  and sgna_ref.version_sequence = version_sequence
