...
using {sp.Sm_Supplier_Org} from '../../sp/sm/SP_SM_SUPPLIER_ORG-model';
using {pg.Vp_Supplier_Mst_View} from '../../pg/vp/PG_VP_SUPPLIER_MST_VIEW-model';
using {sp.Sc_Supplier_Type_View} from '../../sp/sc/SP_SC_REFERENCE_COMMON-model';

@cds.autoexpose  
entity Sc_Sm_Supplier_Org as select from Sm_Supplier_Org 
    mixin {
        supplier_type : Association to Sc_Supplier_Type_View 
...
} where lower(language_cd) = substring($user.locale,1,2)
;
...
using {dp.Mm_Material_Desc_Lng} from '../../dp/mm/DP_MM_MATERIAL_DESC_LNG-model';
