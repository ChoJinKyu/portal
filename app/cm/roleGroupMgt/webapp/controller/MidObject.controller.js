sap.ui.define([
  ...
  "ext/lib/util/Multilingual",
    ...
    return BaseController.extend("cm.roleGroupMgt.controller.MidObject", {
    ...
    onPageEnterFullScreenButtonPress: function () {
      ...
      this.getRouter().navTo("midPage", {
        layout: sNextLayout,
        tenentId: this._sTenantId,
        roleGroupCode: this._sRoleGroupCode
      });
    },
		/**
    ...
    onMidTableDeleteButtonPress: function () {
        ...
        aItems.forEach(function(oItem){
            aIndices.push(oModel.getProperty("/RoleGroupMap").indexOf(oItem.getBindingContext("details").getObject()));
        });
    ...
    onPageSaveButtonPress: function () {
      ...
      if (!master.getData()["tenant_id"]) {
        MessageBox.alert("테넌트ID를 입력하세요.");
        return;
      }
      if (!master.getData()["role_group_code"]) {
        MessageBox.alert("역할그룹코드를 입력하세요.");
        return;
      }
      if (!master.getData()["role_group_name"]) {
        MessageBox.alert("역할그룹명을 입력하세요.");
        return;
      }
      if (!master.getData()["role_desc"]) {
        MessageBox.alert("역할설명을 입력하세요.");
        return;
      }
      ...
      if (master.getData()["_state_"] == "C") {
        detail.getData()["RoleGroupMap"].map(r => {
    ...
    _onMasterDataChanged: function (oEvent) {
      if (this.getModel("midObjectView").getProperty("/isAddedMode") == false) {    // 원래true
        ...
        var sRoleGroupCode = oMasterModel.getProperty("/role_group_code");
        var oDetailsData = oDetailsModel.getData().RoleGroupMap;
 
        oDetailsData.forEach(function (oItem, nIndex) {
            oDetailsModel.setProperty("/" + nIndex + "/tenant_id", sTenantId);
            oDetailsModel.setProperty("/" + nIndex + "/role_group_code", sRoleGroupCode);
        });
    ...
    _onRoutedThisPage: function (oEvent) {
      ...
      this._sRoleGroupCode = oArgs.roleGroupCode;
      if (oArgs.tenantId == "code" && oArgs.roleGroupCode == "roleGroupCode") {
        ...
        this._bindView("/RoleGroupMgt(tenant_id='"+ this._sTenantId+"',role_group_code='"+this._sRoleGroupCode+"')");
        oView.setBusy(true);
      ...
      this.oEditableTemplate = new ColumnListItem({
        cells: [
          ...
          new ComboBox({
            ...
            items: {
              ...
              template: new Item({
                key: "{role_group_code}",
                text: "{= ${role_group_code} + ':' + ${role_group_name}}"
              })
            },
        ...
        _loadFragment: function (sFragmentName, oHandler) {
            if (!this._oFragments[sFragmentName]) {
                Fragment.load({
                    ...
                    name: "cm.roleGroupMgt.view." + sFragmentName,
                    controller: this
        ...
        roleGroupComboChange: function(oEvent) {
            ...
            combo.bindItems({
                ...
                template: new Item({
                    key:"{role_group_code}", text:"{role_group_code} : {role_group_name}"
                })
            });
        }
