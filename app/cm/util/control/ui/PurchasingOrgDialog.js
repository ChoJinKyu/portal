...
sap.ui.define([
    ...
    var PurchasingOrgDialog = Parent.extend("cm.util.control.ui.PurchasingOrgDialog", {

        metadata: {
            properties: {
                loadWhenOpen: { type: "boolean", group: "Misc", defaultValue: false },
                contentWidth: { type: "string", group: "Appearance", defaultValue: "800px"},
                keyField: { type: "string", group: "Misc", defaultValue: "purchase_org_code" },
                textField: { type: "string", group: "Misc", defaultValue: "purchase_org_name" }
            }
        },
        ...
        createSearchFilters: function(){
            return [
                new VBox({
                    items: [
                        new Label({ text: this.getModel("I18N").getText("/PURCHASING_ORG_CODE") +" or "+ this.getModel("I18N").getText("/PURCHASING_ORG_NAME")}),
                        this.oSearchKeyword
                    ],
        ...
        createTableColumns: function(){
            //타이틀 다국어 처리
            this.getProperty("title") ? this.getProperty("title") : this.setProperty("title" , this.getModel("I18N").getText("/ORG_PURCHASING"));
            return [
                new Column({
                    width: "30%",
                    hAlign: "Center",
                    label: new Label({text: this.getModel("I18N").getText("/PURCHASING_ORG_CODE")}),
                    template: new Text({text: "{"+this.getProperty("keyField")+"}"})
                }),
                new Column({
                    width: "70%",
                    label: new Label({text: this.getModel("I18N").getText("/PURCHASING_ORG_NAME")}),
        ...
        loadData: function(){
            ...
            ODataV2ServiceProvider.getService("cm.util.OrgService").read("/Purchasing", {
