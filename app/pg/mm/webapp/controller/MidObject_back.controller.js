       ...
       /**
        ...
        onValueHelpMaterialDialogApply : function () {
            ...
            for(var i=0;i<oSelected.length;i++){
                ...
                //수정대상 현재 개발당시 알수 없는 값들 (20201207)
                //다음 작업은 검색 테이블에서 모든 값을 가져왔을때를 가정한것
                odata.tenant_id = this._sso.dept.tenant_id;
                odata.company_code = this._sso.dept.company_code;
                odata.org_type_code = this._sso.dept.org_type_code;
                odata.org_code = this._sso.dept.org_code;
                odata.itemMode = this._m.itemMode.create;
                odata.odataMode = this._m.odataMode.no;
                odata.base_quantity = "";
                odata.processing_cost="",
                odata.pcst_currency_unit="";
                odata.mi_material_code="";
                odata.mi_material_code_name="";
                odata.category_code="";
                odata.category_name="";
                odata.reqm_quantity_unit="";
                odata.reqm_quantity="";
                odata.currency_unit=""; //currency_unit
                odata.mi_base_reqm_quantity="";
                odata.quantity_unit=""; //quantity_unit
                odata.exchange=""; //exchange
                odata.termsdelv=""; //termsdelv
                odata.use_flag=false;
                odata.local_create_dtm=new Date();
                odata.local_update_dtm=new Date();
                odata.create_user_id=this._sso.user.id;
                odata.update_user_id=this._sso.user.id;
                odata.system_create_dtm=new Date();
                odata.system_update_dtm= new Date();
                //다음정보 상위에서 등록 
                //itemMode
                //odataMode
                //
